...
fn test_e2e_verify() {
    ...
    fn fri_prove_verify(
        ...
        let positions_for_opening = rand_array::<u64, 5>()
            .iter()
            .map(|v| usize::min(*v as usize, lde_blowup * trace_length - 1))
            .collect::<Vec<_>>();
        let opening_proof = prover.open(&positions_for_opening);
        ...
        let result = verify_proof(
            ...
            &evaluations,
            domain_size,
            &positions_for_opening,
            &options,
        );
        assert!(result.is_ok(), "{:?}", result.err().unwrap());

        // make sure proof fails for invalid domain size
        let result_invalid_domain = verify_proof(
            opening_proof,
            proof,
            commitments,
            &evaluations,
        ...
        assert!(result_invalid_domain.is_err());
    }
