...
pub fn run(size: usize, file_path: &str) {
    ...
    let file_directory = Path::new(file_path).parent().expect("Unable to get file directory");
    // Create the directory if it doesn't exist
    if !file_directory.exists() {
        fs::create_dir_all(file_directory).expect("Unable to create directory");
    }
    let mut file = File::create(file_path).expect("Unable to create file");
    file.write_all(&data).expect("Unable to write data");
    println!("Generated data of size {} Bytes and saved to {}", size, file_path);
}
